...
internal sealed class CliApp
{
    ...
    private async Task RenderW2dAsync()
    {
        var input = RequireArg(1, "input .w2d file");
        var output = GetOption("--out") ?? Path.ChangeExtension(input, ".png");
        var options = ReadOptions();
        var renderer = new NativeDwfRenderer();
        var result = await renderer.RenderW2dFileAsync(input, output, options);
        Console.WriteLine(JsonSerializer.Serialize(result, JsonOptions));
        Environment.ExitCode = result.Success ? 0 : 1;
    }
...
Notes:
  ...
  - Native W2D rendering requires a native build compiled with ZS_DWF_ENABLE_ODA_DWFTK=ON.
  - Without the native W2D/DWF renderer, ordinary DWF conversion fails instead of returning incomplete thumbnails/previews.
